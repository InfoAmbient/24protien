  ...
  public function getValuesHtml()
  {
    ...
    if ($_option->getType() == Mage_Catalog_Model_Product_Option::OPTION_TYPE_DROP_DOWN
      ...
      foreach ($_option->getValues() as $_value) {
        ...
        if (isset($default_value[0]['option_type_id']) && ($_value->getOptionTypeId() == $default_value[0]['option_type_id'])) {
          $select->addOption(
    ...
    if ($_option->getType() == Mage_Catalog_Model_Product_Option::OPTION_TYPE_RADIO) {
      $selectHtml = '<ul id="options-' . $_option->getId() . '-list" class="options-list options-list-radio">';
...
//            $selectHtml .= '<li><input type="radio" id="options_' . $_option->getId() . '" class="' . $class . ' product-custom-option" name="options[' . $_option->getId() . ']"' . ($this->getSkipJsReloadPrice() ? '' : ' onclick="opConfig.reloadPrice()"') . ' value="" checked="checked" /><label for="options_' . $_option->getId() . '">' . $this->__('None') . '</label></li>';
//          }
          break;
	...
	if ($_option->getType() == Mage_Catalog_Model_Product_Option::OPTION_TYPE_CHECKBOX) {
      $selectHtml = '<ul id="options-' . $_option->getId() . '-list" class="options-list options-list-checkbox">';
      $require = ($_option->getIsRequire()) ? ' validate-one-required-by-name' : '';
      $arraySign = '';
      switch ($_option->getType()) {
        case Mage_Catalog_Model_Product_Option::OPTION_TYPE_RADIO:
