        ...
        if ($drawPopup) {
            ...
            if(!$ajax_load_popups || $mode == 'mb') {
                if (($level == 0) && ($blockType == 'wide' || $blockType == 'staticwidth') ) {
                    if ($block_top)
                        $html[] = '<div class="top-mega-block"></div>';
                    $html[] = '<div class="mega-columns row">';
                    if ($block_left)
                        $html[] = '<div class="left-mega-block">' . $block_left . '</div>';
        ...
        foreach ($children as $child)
        {
            if (is_object($child) && $child->getIsActive())
            {
                ...
                if ($level == 1) {
                    ...
                    if($block_top and $block_right){
                   $html .= '<div class="col-sm-6">';
                   $html .= '<div class="col-sm-7">';
                   $html .= '<li class="item menu-item-has-children menu-parent-item '.$active.' ">';
                 ...
                 $html .= '<li class="'.$class.' '.$active.' R'.$i.'">';
					//added from version 1.0.2
					//version 1.0.2 start
					if ($type == 'wide' && $catModel->getThumbnail()) {
						$html .= '<div class="menu_thumb_img">';
                    ...
                    $html .= '<li class="'.$class.' '.$active.' R123456">';
                    $html .= '<a class="level' . $level . '" data-id="'.$child->getId().'" href="' . $this->getCategoryUrl($child) . '"><span>' . $name .$label. '</span></a>';
                }
                if (count($activeChildren) > 0)
                {
                ...
                if($block_top and $block_right){
                   $html .= '</div>';
                   $html .= '</li>';
                   $html .= '<div class="col-sm-5"><div class="right-mega-block">' . $block_right . '</div></div>';
                   $html .= '</div>'; 
                }else{
                 $html .= '</li>';   
                }
