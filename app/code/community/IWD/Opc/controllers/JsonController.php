...
class IWD_Opc_JsonController extends Mage_Core_Controller_Front_Action{
	...
	public function saveBillingAction(){
		...
		if ($this->getRequest()->isPost()) {
			...
			//$data['mobile'] = $data['cuntrycode'].$data['mobile'];
			...
			$customerAddressId = $this->getRequest()->getPost('billing_address_id', false);
                       ...
                       $mobilecode = $data['cuntrycode'];
                        $mobilenon = trim($data['mobile']);
                        //$pos = strpos($mobiledata,"@");
                        $mobileno = ltrim($mobilenon, '0');
                        if($mobileno){
                          $mobiledata = $mobilecode.$mobileno;
                           Mage::getSingleton('core/session')->setCustomerMobile($mobiledata);
                        }
			...
			if (isset($data['email'])) {
				$data['email'] = trim($data['email']);
                         ...
                         $data['email'] = trim($data['email']);
                          ...
                          if($data['email']){
                          //$mobiledata = $mobilecode.$mobileno;
                           Mage::getSingleton('core/session')->setCustomerEmail($data['email']);
                        }else{
                           $data['email'] = "&nbsp;";
                        }
