...
jQuery(window).on("load", function() {
  ...
  jQuery.fn[pluginName].windowLoaded = true;
});

function Plugin(element, options) {
  this.telInput = jQuery(element);
  ...
  this.hadInitialPlaceholder = Boolean(jQuery(element).attr("placeholder"));
}
...
Plugin.prototype = {
  ...
  _init: function() {
    ...
    if (this.isMobile) {
      ...
      jQuery("body").addClass("iti-mobile");
    ...
    this.utilsScriptDeferred = new jQuery.Deferred();
  ...
  _generateMarkup: function() {
    ...
    this.telInput.wrap(jQuery("<div>", {
      "class": parentClass
    }));

    this.flagsContainer = jQuery("<div>", {
    ...
    if (this.options.separateDialCode) {
      this.selectedDialCode = jQuery("<div>", {
  ...
  _initRequests: function() {
    ...
    if (this.options.utilsScript) {
      ...
      if (jQuery.fn[pluginName].windowLoaded) {
        jQuery.fn[pluginName].loadUtils(this.options.utilsScript, this.utilsScriptDeferred);
      } else {
        // wait until the load event so we don't block any other requests e.g. the flags image
        jQuery(window).on("load", function() {
          jQuery.fn[pluginName].loadUtils(that.options.utilsScript, that.utilsScriptDeferred);
        });
      }
    } else {
  ...
  _loadAutoCountry: function() {
    ...
    if (jQuery.fn[pluginName].autoCountry) {
      this.handleAutoCountry();
    } else if (!jQuery.fn[pluginName].startedLoadingAutoCountry) {
      // don't do this twice!
      jQuery.fn[pluginName].startedLoadingAutoCountry = true;

      if (typeof this.options.geoIpLookup === 'function') {
        this.options.geoIpLookup(function(countryCode) {
          jQuery.fn[pluginName].autoCountry = countryCode.toLowerCase();
          // tell all instances the auto country is ready
          // TODO: this should just be the current instances
          // UPDATE: use setTimeout in case their geoIpLookup function calls this callback straight away (e.g. if they have already done the geo ip lookup somewhere else). Using setTimeout means that the current thread of execution will finish before executing this, which allows the plugin to finish initialising.
          setTimeout(function() {
            jQuery(".intl-tel-input input").intlTelInput("handleAutoCountry");
          });
        });
      }
    }
  ...
  _initFocusListeners: function() {
    ...
    if (form) {
      jQuery(form).on("submit" + this.ns, function () {
  ...
  _setDropdownPosition: function() {
    ...
    if (!this.isMobile) {
      var pos = this.telInput.offset(),
        ...
        dropdownFitsBelow = (inputTop + this.telInput.outerHeight() + this.dropdownHeight < windowTop + jQuery(window).height()),
      ...
      if (this.options.dropdownContainer) {
        ...
        jQuery(window).on("scroll" + this.ns, function() {
          that._closeDropdown();
        });
      }
    }
  ...
  _updateFlagFromNumber: function(number) {
    ...
    if (dialCode) {
      ...
      var countryCodes = this.countryCodes[this._getNumeric(dialCode)],
        alreadySelected = (jQuery.inArray(this.selectedCountryData.iso2, countryCodes) > -1),
  ...
  _isRegionlessNanp: function(number) {
    ...
    if (numeric.charAt(0) == "1") {
      ...
      return (jQuery.inArray(areaCode, regionlessNanpNumbers) > -1);
    }
    return false;
  },
  ...
  _closeDropdown: function() {
    ...
    // unbind key events
    jQuery(document).off(this.ns);
    // unbind click-off-to-close
    jQuery("html").off(this.ns);
    // unbind hover and click listeners
    this.countryList.off(this.ns);
    // remove menu from container
    if (this.options.dropdownContainer) {
      if (!this.isMobile) {
        jQuery(window).off("scroll" + this.ns);
      }
  ...
  _getFullNumber: function() {
    var val = jQuery.trim(this.telInput.val()),
  ...
  handleAutoCountry: function() {
    if (this.options.initialCountry === "auto") {
      ...
      this.defaultCountry = jQuery.fn[pluginName].autoCountry;
  ...
  isValidNumber: function() {
    var val = jQuery.trim(this._getFullNumber()),
  ...
  if (options === undefined || typeof options === "object") {
    ...
    this.each(function() {
      if (!jQuery.data(this, "plugin_" + pluginName)) {
        ...
        jQuery.data(this, "plugin_" + pluginName, instance);
      }
    });
...
jQuery.fn[pluginName].getCountryData = function() {
  return allCountries;
};


// load the utils script
jQuery.fn[pluginName].loadUtils = function(path, utilsScriptDeferred) {
  if (!jQuery.fn[pluginName].loadedUtilsScript) {
    // don't do this twice! (dont just check if window.intlTelInputUtils exists as if init plugin multiple times in quick succession, it may not have finished loading yet)
    jQuery.fn[pluginName].loadedUtilsScript = true;

    // dont use jQuery.getScript as it prevents caching
    jQuery.ajax({
      type: 'GET',
      url: path,
      complete: function() {
        // tell all instances that the utils request is complete
        jQuery(".intl-tel-input input").intlTelInput("handleUtils");
      },
      dataType: "script",
      cache: true
    });
...
// default options
jQuery.fn[pluginName].defaults = defaults;
