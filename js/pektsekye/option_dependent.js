		...
		for (var i=0;i<l;i++){
			...
			while (ll--){
				if (this.childrenVals.indexOf(this.valsByOption[this.oIds[i]][ll]) == -1){
					...
					this.univValsByOption[this.oIds[i]].unshift(this.valsByOption[this.oIds[i]][ll]);
				}
			}
		...
		$$('.product-custom-option').each(function(element){
			 ...
			 if (element.type == 'radio' || element.type == 'checkbox') {	
				...
				if (element.value != ''){
					...
					this.valsByOption[optionId].unshift(valueId);
		...
		if (this.childrenByOption[id]){		
			...
			while (l--)
				if (optionIds.indexOf(this.optionByValue[ids[l]]) == -1)
					optionIds.unshift(this.optionByValue[ids[l]]);
		}
